...
df = pd.read_excel('../capability100222.xlsx')
interests = ', '.join(df["interests"].to_list()).split(', ')
nodes = [{'id': i, 'group': 1} for i in interests]
for i in range(0, len(df)):
    group = df['Departments'].iloc[i]
    node1 = {"id": df["Scholars"].iloc[i], "group": group}
    nodes.append(node1)
    interests = df["interests"].iloc[i].split(', ')
    for interest in interests:
        link = {"source": df["Scholars"].iloc[i], "target": interest, "value": 10}
        links.append(link)
